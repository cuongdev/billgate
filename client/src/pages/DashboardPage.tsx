...
export const DashboardPage: React.FC = () => {
	const [stats, setStats] = useState<Stats>({
		...
		totalTransactions: 0,
		totalVolume: 0,
		todayVolume: 0, // Not used much, we rely on totalVolume for period
	...
	// Full VND Format: 100.000 đ
	const formatCurrency = (val: number) => {
		return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(val);
	};

	// Helper for Webhook Success Rate
	...
	return (
		<div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500">
						...
						>
							...
							{accounts.map((acc: any) => (
								<option key={acc.keyShare} value={acc.keyShare}>
									{acc.keyShare} {acc.accountNumber ? `- ${acc.accountNumber}` : ''}
								</option>
							))}
						</select>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				{/* 1. Hourly Traffic Heatmap/Bar */}
				<div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-gray-100 shadow-[0_2px_12px_-4px_rgba(6,81,237,0.1)]">
			...
			{/* Row 3: Daily Trend & Transactions */}
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				{/* Main Trend Chart */}
				<div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-gray-100 shadow-[0_2px_12px_-4px_rgba(6,81,237,0.1)]">
					<div className="flex justify-between items-center mb-6">
